...
ranges as (
    select
        ...
        to_date(
            dateadd(week, -52, dateadd(week, -5, a.last_week_start))
        ) as start_6w_yoy,
        to_date(dateadd(week, -52, a.last_week_start)) as end_6w_yoy,
        to_date(
            date_trunc(year, dateadd(day, 6, a.last_week_start))
        ) as ytd_start,
        to_date(
            dateadd(
                year,
        ...
        to_date(
            dateadd(
                year,
                -1,
                to_date(dateadd(day, 6, a.last_week_start))
            )
        ) as last_week_end_yoy,
...
catalog_listings as (
    ...
    from
        analytics_db.stg_catalog.stg_catalog__listings l
        ...
        left join pattern_db.public.product_catalog_brand_hierarchy pt on pt.catalog_brand_id = p.partner_id
...
actuals_sales_cte as (
    select
        ...
        hs.quantity
    from
        pattern_db.public.hourly_sales hs
        left join catalog_listings cl on hs.listing_id = cl.listing_id
    where
        hs.country_code in ('US', 'CA', 'MX', 'BR')
),
weekly_actuals as (
    select
...
metrics_wk_brand as (
    select
        ...
        sum(
            case
                when w.week_start_date between r.start_6w
                and r.last_week_start then w.revenue
                else 0
            end
        ) / 6.0 as last_6w_rev_run_rate,
        sum(
            case
                when w.week_start_date between r.start_6w
                and r.last_week_start then w.units
                else 0
            end
        ) / 6.0 as last_6w_units_run_rate,
        sum(
            case
                when w.week_start_date between r.start_6w_yoy
                and r.end_6w_yoy then w.revenue
                else 0
            end
        ) / 6.0 as last_6w_rev_run_rate_yoy,
        sum(
            case
                when w.week_start_date between r.start_6w_yoy
                and r.end_6w_yoy then w.units
                else 0
            end
        ) / 6.0 as last_6w_units_run_rate_yoy,
        nullif(
            (
                sum(
                    case
            ...
            0
        ) as last_6w_rev_run_rate_yoy_pct,
        nullif(
            (
                sum(
                    case
        ...
        sum(
            case
                when w.week_start_date = r.last_week_start then w.revenue
                else 0
            end
        ) as last_week_rev,
        sum(
            case
                when w.week_start_date = r.last_week_start then w.units
                else 0
            end
        ) as last_week_units,
        sum(
            case
                when w.week_start_date = dateadd(week, -52, r.last_week_start) then w.revenue
                else 0
            end
        ) as last_week_rev_yoy,
        sum(
            case
                when w.week_start_date = dateadd(week, -52, r.last_week_start) then w.units
                else 0
            end
        ) as last_week_units_yoy,
        nullif(
            sum(
                case
                    when w.week_start_date = r.last_week_start then w.revenue
...
ytd_daily_brand as (
    select
        ...
        sum(
            case
                when s.order_date between r.ytd_start
                and r.last_week_end then s.converted_revenue
                else 0
            end
        ) as ytd_rev,
        sum(
            case
                when s.order_date between r.ytd_start
                and r.last_week_end then s.quantity
                else 0
            end
        ) as ytd_units,
        sum(
            case
                when s.order_date between r.ytd_start_yoy
                and r.last_week_end_yoy then s.converted_revenue
                else 0
            end
        ) as ytd_rev_yoy,
        sum(
            case
                when s.order_date between r.ytd_start_yoy
                and r.last_week_end_yoy then s.quantity
                else 0
            end
        ) as ytd_units_yoy
    from
...
im_base_filtered as (
    select
        ...
        b.marketplace_id
    from
        pattern_db.inventory_hub.demand_forecasts b
        cross join ranges r
    where
        b.week_start >= r.start_13w
        and b.week_start < r.end_13w_excl
    group by
        all
),
im_all_items as (
    select
        b.master_id,
        dateadd(day, seq.seq, b.week_start) as date,
        b.week_start,
        b.marketplace_id
    from
        im_base_filtered b
...
im_forecast as (
    select
        ...
        (
            coalesce(df.base_rate_overwrite, df.base_rate, 0) + coalesce(
                df.promo_increase_overwrite,
                df.promo_increase,
                0
            ) + coalesce(
                df.seasonal_increase_overwrite,
                df.seasonal_increase,
                0
            )
        ) / 7 as quantity
    from
        im_all_items a
        join pattern_db.inventory_hub.demand_forecasts df on df.master_id = a.master_id
    ...
    where
        df.effective_end_date is null
        and exists (
            select
                1
            from
                allowed_marketplaces am
            where
                am.marketplace_name = mm.name
        )
),
...
im_fcst_13w_brand as (
    select
        f.catalog_brand,
        sum(f.fcst_units) as fcst_units_13w_im
    from
        im_forecast_weekly_brand f
        cross join ranges r
    where
        f.week_start_date >= r.start_13w
        and f.week_start_date < r.end_13w_excl
    group by
        1
),
act_yoy_for_fcst_13w as (
    ...
    from
        ...
        cross join ranges r
    where
        w.week_start_date >= dateadd(week, -52, r.start_13w)
...
ae_bounds as (
    select
        ...
        dateadd(
            month,
            1,
            date_trunc('month', dateadd(day, -1, r.end_13w_excl))
        ) as ae_month_end_excl,
...
ae_forecast_src as (
    select
        ...
        units
    from
        PATTERN_DB.ACCOUNTING_FINANCE.ADAPTIVE_OP_2026_UNITS
),
ae_monthly_forecast as (
    select
        s.brand,
        s.month_start,
        sum(s.units) as forecast_units
    from
        ae_forecast_src s
        cross join ae_bounds b
    where
        s.month_start >= b.ae_month_start
        and s.month_start < b.ae_month_end_excl
    group by
        1,
        2
),
ae_prev_daily as (
    ...
    from
        ...
        cross join ae_bounds b
    where
        a.order_date >= b.prev_month_start
        and a.order_date < b.prev_month_end_excl
    group by
        1,
        2
),
ae_monthly_totals_prev as (
    select
        brand,
        date_trunc('month', dt) as month_start_prev,
...
ae_shares_brand_prev as (
    select
        ...
        case
            when mt.month_qty > 0 then coalesce(pd.qty, 0)::float / mt.month_qty
            else null
        end as share_brand
    from
        ae_monthly_totals_prev mt
        join ae_calendar_prev cp on date_trunc('month', cp.dt) = mt.month_start_prev
        left join ae_prev_daily pd on pd.brand = mt.brand
        and pd.dt = cp.dt
),
ae_avg_shares_prev as (
    select
        month_no,
        day_of_month,
        avg(share_brand) as share_all_brands
    from
        ae_shares_brand_prev
    where
        share_brand is not null
    group by
        1,
        2
),
...
ae_daily_alloc as (
    select
        mf.brand,
        dh.week_start,
        mf.forecast_units * coalesce(
            sb.share_brand,
            av.share_all_brands,
            1.0 / dh.days_in_month_yr
        ) as daily_forecast_units
    from
        ae_days_horizon dh
        join ae_monthly_forecast mf on dh.month_start_yr = mf.month_start
        left join ae_shares_brand_prev sb on sb.brand = mf.brand
        and sb.month_no = dh.month_no
        and sb.day_of_month = dh.day_of_month
        left join ae_avg_shares_prev av on av.month_no = dh.month_no
        and av.day_of_month = dh.day_of_month
),
ae_weekly_forecast as (
    select
        brand as catalog_brand,
        week_start,
        sum(daily_forecast_units) as weekly_forecast_units
    from
        ae_daily_alloc
    group by
        1,
...
ae_fcst_13w_brand as (
    select
        w.catalog_brand,
        sum(w.weekly_forecast_units) as fcst_units_13w_ae
    from
        ae_weekly_forecast w
        cross join ranges r
    where
        w.week_start >= r.start_13w
        and w.week_start < r.end_13w_excl
    group by
        1
),
inv_max as (
    select
        max(date) as inv_date
    from
        pattern_db.inventory_hub.daily_item_inventories
    where
        region in ('US', 'CA')
...
inventory_base as (
    ...
    from
        pattern_db.inventory_hub.daily_item_inventories i
        join inv_max m on i.date = m.inv_date
        join analytics_db.stg_catalog.stg_catalog__products pr on pr.master_id = i.master_id
        join pattern_db.public.product_catalog_brand_hierarchy pt on pt.catalog_brand_id = pr.partner_id
        and is_terminal_level = true
...
inventory_brand as (
    select
        catalog_brand,
        date as inv_asof_date,
        sum(quantity) as inv_units,
        sum(inventory_value_usd) as inv_value_usd
    from
        inventory_base
    group by
        1,
...
brands_universe as (
    ...
    union
    select
        distinct brand as catalog_brand
    from
        ae_forecast_src
    union
    select
...
combined_brand as (
    select
        b.catalog_brand,
        coalesce(
            m.last_completed_week_start,
            y.last_completed_week_start
        ) as last_completed_week_start,
        ...
        case
            when y.ytd_rev_yoy is null
            or y.ytd_rev_yoy = 0 then null
            else (y.ytd_rev - y.ytd_rev_yoy) / y.ytd_rev_yoy
        end as ytd_rev_yoy_pct,
        case
            when y.ytd_units_yoy is null
            or y.ytd_units_yoy = 0 then null
            else (y.ytd_units - y.ytd_units_yoy) / y.ytd_units_yoy
        ...
        case
            when a.units_13w_yoy_actuals is null then null
            else a.units_13w_yoy_actuals / 13.0
        end as fcst_units_run_rate_yoy_13w_im,
        case
            when a.units_13w_yoy_actuals is null
            or a.units_13w_yoy_actuals = 0 then null
            else (
                coalesce(im.fcst_units_13w_im, 0) - a.units_13w_yoy_actuals
            ) / a.units_13w_yoy_actuals
        ...
        end as fcst_units_run_rate_yoy_13w_ae,
        case
            when a.units_13w_yoy_actuals is null
            or a.units_13w_yoy_actuals = 0 then null
            else (
                coalesce(ae.fcst_units_13w_ae, 0) - a.units_13w_yoy_actuals
            ) / a.units_13w_yoy_actuals
        ...
        case
            when (coalesce(im.fcst_units_13w_im, 0) / 13.0) = 0 then null
            else coalesce(inv.inv_units, 0) / (coalesce(im.fcst_units_13w_im, 0) / 13.0)
        end as wos_weeks_im,
        case
            when (coalesce(ae.fcst_units_13w_ae, 0) / 13.0) = 0 then null
            else coalesce(inv.inv_units, 0) / (coalesce(ae.fcst_units_13w_ae, 0) / 13.0)
        end as wos_weeks_ae
    from
        brands_universe b
...
combined_total as (
    select
        ...
        case
            when sum(last_6w_rev_run_rate_yoy) = 0 then null
            else (
                sum(last_6w_rev_run_rate) - sum(last_6w_rev_run_rate_yoy)
            ) / sum(last_6w_rev_run_rate_yoy)
        end as last_6w_rev_run_rate_yoy_pct,
        case
            when sum(last_6w_units_run_rate_yoy) = 0 then null
            else (
                sum(last_6w_units_run_rate) - sum(last_6w_units_run_rate_yoy)
            ) / sum(last_6w_units_run_rate_yoy)
        ...
        case
            when sum(last_week_rev_yoy) = 0 then null
            else (sum(last_week_rev) - sum(last_week_rev_yoy)) / sum(last_week_rev_yoy)
        end as last_week_rev_yoy_pct,
        case
            when sum(last_week_units_yoy) = 0 then null
        ...
        case
            when sum(ytd_rev_yoy) = 0 then null
            else (sum(ytd_rev) - sum(ytd_rev_yoy)) / sum(ytd_rev_yoy)
        end as ytd_rev_yoy_pct,
        case
            when sum(ytd_units_yoy) = 0 then null
        ...
        case
            when sum(coalesce(fcst_units_run_rate_yoy_13w_im, 0)) = 0 then null
            else (
                (sum(fcst_units_13w_im) / 13.0) - sum(coalesce(fcst_units_run_rate_yoy_13w_im, 0))
        ...
        case
            when sum(coalesce(fcst_units_run_rate_yoy_13w_ae, 0)) = 0 then null
            else (
                (sum(fcst_units_13w_ae) / 13.0) - sum(coalesce(fcst_units_run_rate_yoy_13w_ae, 0))
            ) / sum(coalesce(fcst_units_run_rate_yoy_13w_ae, 0))
        end as fcst_units_run_rate_yoy_pct_13w_ae,
        sum(inv_units) as inv_units,
        sum(inv_value_usd) as inv_value_usd,
        case
            when sum(fcst_units_13w_im) / 13.0 = 0 then null
            else sum(inv_units) / (sum(fcst_units_13w_im) / 13.0)
        end as wos_weeks_im,
        case
            when sum(fcst_units_13w_ae) / 13.0 = 0 then null
            else sum(inv_units) / (sum(fcst_units_13w_ae) / 13.0)
        end as wos_weeks_ae
    from
        combined_brand
),
...
select
    ...
    last_completed_week_start,
    to_char(
        round(last_6w_rev_run_rate),
        'FM999,999,999,999,999'
    ) as last_6w_rev_run_rate,
    to_char(
        round(last_6w_units_run_rate),
        'FM999,999,999,999,999'
    ) as last_6w_units_run_rate,
    to_char(
        round(last_6w_rev_run_rate_yoy),
        'FM999,999,999,999,999'
    ) as last_6w_rev_run_rate_yoy,
    to_char(
        round(last_6w_units_run_rate_yoy),
        'FM999,999,999,999,999'
    ) as last_6w_units_run_rate_yoy,
    case
        when last_6w_rev_run_rate_yoy is null
        or last_6w_rev_run_rate_yoy = 0 then null
        else to_char(
            round(
                100 *(last_6w_rev_run_rate - last_6w_rev_run_rate_yoy) / last_6w_rev_run_rate_yoy
            ),
            'FM999,999,990'
        ) || '%'
    end as last_6w_rev_run_rate_yoy_pct,
    case
        when last_6w_units_run_rate_yoy is null
        or last_6w_units_run_rate_yoy = 0 then null
        else to_char(
            round(
                100 *(
                    last_6w_units_run_rate - last_6w_units_run_rate_yoy
                ) / last_6w_units_run_rate_yoy
            ),
            'FM999,999,990'
        ) || '%'
    ...
    to_char(
        round(last_week_rev_yoy),
        'FM999,999,999,999,999'
    ) as last_week_rev_yoy,
    to_char(
        round(last_week_units_yoy),
        'FM999,999,999,999,999'
    ) as last_week_units_yoy,
    case
        when last_week_rev_yoy is null
        or last_week_rev_yoy = 0 then null
        else to_char(
            round(
                100 *(last_week_rev - last_week_rev_yoy) / last_week_rev_yoy
            ),
            'FM999,999,990'
        ) || '%'
    end as last_week_rev_yoy_pct,
    case
        when last_week_units_yoy is null
        or last_week_units_yoy = 0 then null
        else to_char(
            round(
                100 *(last_week_units - last_week_units_yoy) / last_week_units_yoy
            ),
            'FM999,999,990'
        ) || '%'
    end as last_week_units_yoy_pct,
    ...
    case
        when ytd_rev_yoy is null
        or ytd_rev_yoy = 0 then null
        else to_char(
            round(100 *(ytd_rev - ytd_rev_yoy) / ytd_rev_yoy),
            'FM999,999,990'
        ) || '%'
    end as ytd_rev_yoy_pct,
    case
        when ytd_units_yoy is null
        or ytd_units_yoy = 0 then null
        else to_char(
            round(100 *(ytd_units - ytd_units_yoy) / ytd_units_yoy),
            'FM999,999,990'
        ) || '%'
    end as ytd_units_yoy_pct,
    to_char(
        round(fcst_units_13w_im),
        'FM999,999,999,999,999'
    ) as fcst_13w_units_total_im,
    to_char(
        round(fcst_units_run_rate_13w_im),
        'FM999,999,999,999,999'
    ) as fcst_13w_units_run_rate_im,
    to_char(
        round(fcst_units_run_rate_yoy_13w_im),
        'FM999,999,999,999,999'
    ) as fcst_13w_units_run_rate_yoy_im,
    case
        when fcst_units_run_rate_yoy_13w_im is null
        or fcst_units_run_rate_yoy_13w_im = 0 then null
        else to_char(
            round(
                100 *(
                    fcst_units_run_rate_13w_im - fcst_units_run_rate_yoy_13w_im
                ) / fcst_units_run_rate_yoy_13w_im
            ),
            'FM999,999,990'
        ) || '%'
    end as fcst_13w_units_run_rate_yoy_pct_im,
    to_char(
        round(fcst_units_13w_ae),
        'FM999,999,999,999,999'
    ) as fcst_13w_units_total_ae,
    to_char(
        round(fcst_units_run_rate_13w_ae),
        'FM999,999,999,999,999'
    ) as fcst_13w_units_run_rate_ae,
    to_char(
        round(fcst_units_run_rate_yoy_13w_ae),
        'FM999,999,999,999,999'
    ) as fcst_13w_units_run_rate_yoy_ae,
    case
        when fcst_units_run_rate_yoy_13w_ae is null
        or fcst_units_run_rate_yoy_13w_ae = 0 then null
        else to_char(
            round(
                100 *(
                    fcst_units_run_rate_13w_ae - fcst_units_run_rate_yoy_13w_ae
                ) / fcst_units_run_rate_yoy_13w_ae
            ),
            'FM999,999,990'
        ) || '%'
    end as fcst_13w_units_run_rate_yoy_pct_ae,
